...
# Segments are also proportional to county pop 
...
adj_pop = countypop$POP10*cluster_RR # Adjust segments by county pop
breakpts = c(0,cumsum(adj_pop)/sum(adj_pop)) # Set intervals accordingly

altdata_heterog = matrix( , C, T)
for (j in 1:T)
{
altdata_heterog[ , j] = table(cut(runif(N, 0, 1),
	breaks = breakpts))/
	countypop$POP10
}
return(altdata_heterog)
}
...
{ filename = paste("DATA_Local cluster_", CS[i], "%_heterog.csv")
