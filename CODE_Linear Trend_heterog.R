...
# Assumptions: Heterogenous populations across counties
...
P_homog = 5000	# County pop under homogenous pop
N_homog = 5000	# Total number of national cases under homog pop
N = (N_homog/(C*P_homog))*	# Keep same avg national rate as under homog pop
...
assign_lrisk = l_risk[counties]*countypop$POP10

# List breakpoints according to assigned risk
breakpts_l = c(0, cumsum(assign_lrisk)/sum(assign_lrisk))

# Create alt data
altlinear_heterog = matrix( , C, T)	
for (j in 1:T)
{
altlinear_heterog[ , j] = table(cut(runif(N, 0, 1),	# Generate runif RV
	breaks = breakpts_l))/	# Cut according to breakpts defined by risk				
	countypop$POP10	# Normalize by Census 2010 pop
}

export_linear = cbind(countypop, altlinear_heterog)
write.table(export_linear, file="DATA_ Alt Linear Heterog.csv", 
	sep = ",", row.names = FALSE)
...
# Assign counties to risks based on longitude
# Adjust risk by population
assign_exp = exp_risk[counties]*countypop$POP10

# List breakpoints according to assigned risk
breakpts_exp = c(0, cumsum(assign_exp)/sum(assign_exp))

# Create alt data
altexp_heterog = matrix( , C, T)	
for (j in 1:T)
{
altexp_heterog[ , j] = table(cut(runif(N, 0, 1),
	breaks = breakpts_exp))/					
	countypop$POP10
}

export_exp = cbind(countypop, altexp_heterog)
write.table(export_exp, file="DATA_Alt Exponential Heterog.csv", 
	sep = ",", row.names = FALSE)

###################### Test Plots ######################

# Get the mean count generated for each county
mean_lrate = rowMeans(altlinear_heterog)
mean_exp = rowMeans(altexp_heterog)
...
# par(mfrow=c(1,2))
# plot(countypop$longitude, countypop$latitude, main = "Linear Trend Heterog",
	# col = color[cut_lrate])
# plot(countypop$longitude, countypop$latitude, main = "Exp Trend Heterog",
